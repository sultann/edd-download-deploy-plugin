<?php
/**
 * Plugin Name:       Easy Digital Downloads - Download deploy
 * Plugin URI:        https://github.com/sultann/edd-download-deploy-plugin
 * Description:       Companion plugin of https://github.com/sultann/edd-download-deploy to deploy downloads from GitHub.
 * Version:           1.0.0
 ...
 * Text Domain:       edd-download-deploy
 * Domain Path:       /languages
 *
 * @package           EDDDownloadDeploy
 */

namespace EDDDownloadDeploy;
...
function add_query_mode( $modes ) {
	$modes[] = 'download-deploy';

	return $modes;
}

add_filter( 'edd_api_output_data', __NAMESPACE__ . '\handle_deploy_action', 10, 2 );

/**
 * Handle the GitHub deploy query.
 *
 ...
 * @return array
 */
function handle_deploy_action( $data, $endpoint ) {
	if ( 'download-deploy' !== $endpoint ) {
		return $data;
	}
	...
	$changelog = isset( $_REQUEST['changelog'] ) ? sanitize_textarea_field( wp_unslash( $_REQUEST['changelog'] ) ) : '';
